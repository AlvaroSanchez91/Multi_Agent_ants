...
hormigas-own [ vida ataque velocidad rango ]
humanos-own [ vida ataque velocidad rango ]

to setup
  clear-all
  reset-ticks
  import-drawing "ruinas.jpg"
  create-humanos supervivientes [
    setxy random-xcor random-ycor
  ]
  create-hormigas hormigas_mutantes [
    setxy random-xcor random-ycor
  ]
  ask hormigas [ set shape "bug"
 set color 35
 set vida vida_hormigas
 set ataque ataque_hormigas
 set velocidad velocidad_hormigas
 set rango rango_hormigas
  ]


  ask humanos [ set shape "person"
 set color 9.9
 set vida vida_humanos
 set ataque ataque_humanos
 set velocidad velocidad_humanos
 set rango rango_humanos
  ]
  if Lider = True [
    ask humanos with [who = 1 ] [
    set color 65
    set ataque ataque_humanos * 2
    ]
  ]
  if Monstruo = True [
    ask hormigas with [who = ((count humanos)  ) ] [
    set color 65
    set ataque ataque_monstruo
    set size 4
    set color 13
    set vida vida_monstruo
    ]
  ]
end
...
to go
  ask hormigas [
    if vida < 0 [die]
    ifelse count humanos > 0 [
      let closest-humano min-one-of (humanos) [distance myself]
      let dist distance (closest-humano)
      ifelse dist < vision_hormigas [
        ifelse vida < covardia_hormigas [ set ataque ataque_hormigas / 2 ]
          [ set heading ( towards closest-humano) ]
      ]
        [set heading(heading + (angle_hormigas / 2 ) - ( random angle_hormigas ))]
      let ataque_efectuado ataque
      ifelse dist < rango [ask closest-humano [set vida (vida - random ataque_efectuado ) ]] []
    ] [set heading(heading + (angle_hormigas / 2 ) - ( random angle_hormigas ))]
...
ask humanos [
    if vida < 0 [die]
    ifelse count hormigas > 0[
      let closest-hormiga min-one-of (hormigas) [distance myself]
      let dist distance (closest-hormiga)
      ifelse dist < vision_humanos [
        ifelse vida < covardia_humanos [ set ataque ataque_humanos / 2 ]
        [ set heading ( towards closest-hormiga) ] ]
        [set heading(heading + (angle_humanos / 2 ) - ( random angle_humanos ))]
      let ataque_efectuado ataque
      ifelse dist < rango [ask closest-hormiga [set vida (vida -  random ataque_efectuado ) ]] []
    ][set heading(heading + (angle_humanos / 2 ) - ( random angle_humanos ))]

    if Lider = True[
      if who != 1 [
      if count humanos with [who = 1 ] > 0[
        let el_lider one-of humanos with [who = 1 ]
    let dist_lider distance ( el_lider)
    if dist_lider < vision_humanos [ set heading ( towards el_lider)]
    ]
    ]
    ]
    forward velocidad
